 ...
 *
 * Audits internal routing links in breed-link-map.json against actual blog
...
/** Returns the correct slug if a blog post exists for this breed+type, else null */
...
for (const [breed, links] of Object.entries(linkMap)) {
  for (const linkType of BLOG_LINK_TYPES) {
    const url = links[linkType];
    if (!url) continue;  // null/missing — handled separately below

    const mapSlug = url.replace('/blog/', '').replace(/^\//, '').replace(/\/$/, '');
...
// ─── audit orphaned posts (blog file exists but map has null for that type) ──
...
const byType = {};
for (const lt of BLOG_LINK_TYPES) {
  byType[lt] = {
...
for (const { breed, linkType, correctSlug } of fixable) {
  if (linkMap[breed]) {
    linkMap[breed][linkType] = `/blog/${correctSlug}`;
    fixedCount++;
  }
}
...
// Set to false where no blog post exists (was true but URL is broken/missing)
...
for (const { mapSlug, correctSlug } of fixable) {
  const from = `/blog/${mapSlug}`;
  const to = `/blog/${correctSlug}`;
...
if (newRedirectLines.length > 0) {
  const block = '\n# Blog slug redirects — generated by audit-routing.mjs\n'
